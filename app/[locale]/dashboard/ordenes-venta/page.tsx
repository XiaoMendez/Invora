...
type OVStatus = 'borrador' | 'confirmada' | 'entregada' | 'cancelada'
...
const STATUS_COLORS: Record<OVStatus, string> = {
...
const EMPTY_ITEM = (): Omit<OVItem, 'id' | 'subtotal' | 'producto'> => ({
  id_producto: '',
  descripcion: '',
  cantidad: 1,
  precio_unitario: 0,
...
export default function OrdenesVentaPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<OVStatus | 'todas'>('todas')
  const [dialogOpen, setDialogOpen] = useState(false)
  const [editingOV, setEditingOV] = useState<SalesOrder | null>(null)
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState<string | null>(null)

  const { data: ordenesData, error: ordenesError, isLoading: ordenesLoading } = useSWR('/api/ordenes-venta', fetcher)
  const { data: clientesData } = useSWR('/api/clientes', fetcher)
  const { data: productosData } = useSWR('/api/productos', fetcher)

  const ordenes: SalesOrder[] = ordenesData?.ordenes_venta || []
  ...
  const filteredOrdenes = ordenes.filter((o) => {
    const clienteNombre = `${o.cliente?.nombre || ''} ${o.cliente?.apellido || ''}`.trim()
    const matchesSearch =
      o.numero_ov.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const resetForm = () => {
    setFormData({ id_cliente: '', fecha_entrega_esperada: '', notas: '', estado: 'borrador' })
    setItems([EMPTY_ITEM()])
    setEditingOV(null)
    setError(null)
  }
  ...
  const handleEdit = (ov: SalesOrder) => {
    setEditingOV(ov)
    setFormData({
      id_cliente: ov.id_cliente,
      fecha_entrega_esperada: ov.fecha_entrega_esperada || '',
      notas: ov.notas || '',
      estado: ov.estado,
    })
    setItems(
      ov.ordenes_venta_items.length > 0
        ? ov.ordenes_venta_items.map((i) => ({
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/ordenes-venta/${id}`, { method: 'DELETE' })
      if (res.ok) {
        mutate('/api/ordenes-venta')
      } else {
    ...
    const updated = items.map((item, i) => {
      ...
      if (field === 'id_producto' && value) {
        ...
        if (prod) {
          ...
          newItem.precio_unitario = parseFloat(prod.precio_venta) || 0
        }
      }
      return newItem
    })
  ...
  const handleSave = async () => {
    ...
    try {
      const method = editingOV ? 'PUT' : 'POST'
      const url = editingOV ? `/api/ordenes-venta/${editingOV.id}` : '/api/ordenes-venta'
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Gestiona tus ordenes de venta a clientes</p>
        </div>
        ...
        <CardContent className="pt-4">
          <div className="flex gap-3">
            <div className="relative flex-1">
              ...
              <Input
                placeholder="Buscar por numero OV o cliente..."
                value={searchQuery}
            ...
            <Select value={filterStatus} onValueChange={(v: any) => setFilterStatus(v)}>
              ...
              <SelectContent>
                ...
                {(Object.keys(STATUS_LABELS) as OVStatus[]).map((s) => (
        ...
        <CardContent className="p-0">
          {ordenesLoading ? (
            ...
            <div className="p-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">No hay ordenes de venta registradas</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
                <TableRow className="border-b border-border/30">
                  <TableHead className="text-xs">Numero OV</TableHead>
              ...
              <TableBody>
                {filteredOrdenes.map((orden) => (
                  <TableRow key={orden.id} className="border-b border-border/20 hover:bg-secondary/30">
                    <TableCell className="text-sm font-mono">{orden.numero_ov}</TableCell>
                    <TableCell className="text-sm">
                      {orden.cliente ? `${orden.cliente.nombre} ${orden.cliente.apellido || ''}`.trim() : 'N/A'}
                    </TableCell>
      ...
      <Dialog open={dialogOpen} onOpenChange={(open) => { setDialogOpen(open); if (!open) resetForm() }}>
        <DialogContent className="glass-card border-border/30 sm:max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingOV ? 'Editar Orden de Venta' : 'Nueva Orden de Venta'}</DialogTitle>
            <DialogDescription>
              {editingOV ? `Editando ${editingOV.numero_ov}` : 'Crea una nueva orden de venta para un cliente'}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave} disabled={loading}
              ...
              {editingOV ? 'Guardar Cambios' : 'Crear Orden'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
